...
class X32 extends EventEmitter {
    ...
    constructor(config) {
        ...
        for (const [name, action] of Object.entries(this.actions)) {
            ...
            // CONSIDER duplicate address protection
        }
    ...
    poll() {
        ...
        setTimeout(() => { 
            console.log(DEBUG_PREFIX, "poll cycles: ", this.pollCyclesCounter);
            if (this.pollCyclesCounter >= POLL_CYCLES_THRESHOLD) {
            // this.emit("x32GoneSilent");
            // [ ] TODO
            // perhaps dim the 'LED' rather than set it to 'off' or disabling button
            }
    ...
    onMessage(message) {
        this.pollCyclesCounter = 0;
        // TODO this.emit("x32GoneSilent", false) 
    ...
    executeToggle(actionName) {
        ...
        setTimeout(() => { this.send(action.address); }, 0); // no delay seems OK
    }
