...
class X32 extends EventEmitter {
    ...
    poll() {
        ...
        // [ ] TODO link the poll to onMessage
        // perhaps if last message received by onMessage is older than `pollIntervalMs`
        // it implies X32 is disconnected, therefore warn or disable buttons?
        // should we do this with a question/answer flag between poll and onMessage?
        // - xremote sent, [onMessage cancels flag]
        // - at next poll if flag not cancelled it implies no message received
        // - either treat immediately as disconnection response, or, use flag as counter and treat `n` cycles as disconnection
        // - then the server / html also need to respond
    }
    ...
    onMessage(message) {
        ...
        value = Boolean(value);
        if (action.invertState) { value = !value; }
    ...
    handleLoadResponse(args) {
        ...
        if ( typeof type === "object" && type !== null && "value" in type ) { type = type.value; }
        if ( typeof result === "object" && result !== null && "value" in result ) { result = result.value; }

        console.log( DEBUG_PREFIX, `Load response: ${type} => ${result}` );
    ...
    executeAction(actionName) {
        ...
        if (!action) {
            console.warn( DEBUG_PREFIX, `Unknown action: ${actionName}` );
            return;
        }
        ...
        switch (action.type) {
            ...
            default:
                console.warn( DEBUG_PREFIX, `Unsupported action type: ${action.type}` );
        }
    }
    ...
    executeToggle(actionName) {
        ...
        if (current === null) {
            console.warn( DEBUG_PREFIX, `Cannot toggle ${actionName}: state unknown` );
            return;
        }
    ...
    executeSnippet(action) {
        ...
        console.log(DEBUG_PREFIX, `Loading snippet ${snippet}`);
