...
class OBS extends EventEmitter {
  ...
  async reconnect() {
    ...
    // TODO do we really need a manual reconnect (and hence disconnect)?
  ...
  async onSceneChanged(event) {
    ...
    if (true) {
      ...
      if (event.sceneName == this.OVERLAY_SCENENAME) {
        // on entry
        this.emit('overlaySceneSelected');
...
console.log(DEBUG_PREFIX, `ptz_action device_id: ${device_id} action: ${ptz_action} preset_id: ${preset_id}`);
  ...
  async onceExitStarted() {
    ...
    // TODO probably better to emit to server, and server to handle the plan, rather than directly to camera
