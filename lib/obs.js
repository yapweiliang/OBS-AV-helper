  ...
  async connect() {
    ...
    try {
      ...
      if (error.code == 4009) {
        ...
        this.scheduleReconnect();
        // don't bother to scheduleReconnect?
        // TODO how to post this as a status message on clients?
      } else {
...
console.log(debugPrefix(), `ptz_action device_id: ${device_id} action: ${ptz_action} preset_id: ${preset_id}`);
  ...
  async updateOverlayCache() {
    ...
    if (!this.obsConnectSuccess) return;
    try {
      const scenesList = await this.obs.call('GetSceneList');
      const exists = scenesList.scenes.some(s => s.sceneName === this.OVERLAY_SCENENAME);
      if (!exists) {
  ...
  async setSourceVisible(sourceName, visible) {
    ...
    try {
      ...
      console.error(debugPrefix(), "setSourceVisible", error);
    }
  }
  ...
  async setTextSourceText(sourceName, text) {
    ...
    try {
      ...
      console.error(debugPrefix(), "setTextSourceText", error);
    }
  }
  ...
  async getTextSourceText(sourceName) {
    ...
    try {
      ...
      console.error(debugPrefix(), "getTextSourceText", error);
    }
  }
  ...
  async getCurrentProgramScene() {
    ...
    try {
      ...
      console.error(debugPrefix(), "getCurrentProgramScene", error)
    }
  }
  ...
  async setCurrentProgramScene(sceneName) {
    ...
    try {
      ...
      if (error.code == 600) {
        ...
        console.error(debugPrefix(), "setCurrentProgramScene", error)
      }
      return error.code;
    }
  }
