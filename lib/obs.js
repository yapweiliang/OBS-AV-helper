...
class OBS extends EventEmitter {
  ...
  async connect() {
    ...
    if (this.obsConnectSuccess) {
      this.obsConnectReport = 'Connected to OBS'    // TODO work in progress
...
console.log(debugPrefix(), `ptz_action device_id: ${device_id} action: ${ptz_action} preset_id: ${preset_id}`);
  ...
  async updateOverlayCacheAndScenesReport() {
    ...
    try {
      ...
      const invalidScenes = this.sceneNames.filter( sceneName => !obsScenesSet.has(sceneName));
      if (invalidScenes.length === 0) {
        this.obsScenesReport = `No mis-spelt scenes`;
      } else {
        this.obsScenesReport = invalidScenes;
