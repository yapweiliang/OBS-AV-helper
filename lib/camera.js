...
class camera extends EventEmitter {
    ...
    async callAPI(jsonInput, urlOption = SET_INFO, timeoutMultiplier = 1) {
        return new Promise((resolve) => {
            this.apiQueue.push({ jsonInput, urlOption, timeoutMultiplier, resolve });
            if (!this.apiBusy) {
                this.processAPIQueue();
            } else {
                console.log(debugPrefix(), `Camera API queue length = ${this.apiQueue.length}`);
            }
        });
    }
    ...
    async processAPIQueue() {
        ...
        while (this.apiQueue.length > 0) {
            const job = this.apiQueue.shift();
            const result = await this._callAPI( job.jsonInput, job.urlOption, job.timeoutMultiplier );
            job.resolve(result);
        }
        this.apiBusy = false;
    }

    async _callAPI(jsonInput, urlOption = SET_INFO, timeoutMultiplier = 1) {
...
//            this.apiBusy = false;
    ...
    async wakeUpCamera() {
        ...
        const e = await this.callAPI(j_camera_wakeup, undefined, 7);
        return e;
    }
    ...
    async shutdownCamera() {
        ...
        const e = await this.callAPI(j_camera_sleep, undefined, 7); // 5 might not be enough, try 7
        return e;
    }
