...
class camera extends EventEmitter {
    ...
    async rebootCamera() {
        // updating of UI should be done by server.js
        this.lastSelectedPreset = -1;
        const e = await this.callAPI(j_camera_reboot, undefined, 2);
        return e;
    }

    async wakeUpCamera() {
        // updating of UI should be done by server.js
        const e = await this.callAPI(j_camera_wakeup, undefined, 2);
console.log(debugPrefix(), "wakeUpCamera()", e)
        return e;
    }

    async shutdownCamera() {
        // updating of UI should be done by server.js
        const e = await this.callAPI(j_camera_sleep, undefined, 2);
        return e;
    }

    async isCameraResponding() {
        const result = await callAPI(j_camera_get_output_info, GET_INFO);
...
*/
        const e = await this.callAPI(j);
        if (e) {
            const newColor = await this.getCameraTallyColor();
            console.log(debugPrefix(), `TALLY COLOR: requested "${color}", result "${newColor}"`); // TODO let's watch and find out
            if (newColor == color) {
                this.emit("updateClientTallyLightIndicator", color);
                return true; // success
            }
        } else {
            console.warn(debugPrefix(), "TALLY_COLOR_UNKNOWN")
