...
class camera extends EventEmitter {
    ...
    async onePushFocus() {
        ...
        this.emit("flashStatusText", {text: msg});
        const e = await this.callAPI(ja_camera_do_onepush_focus); // includes j_camera_set_af_lock_ff
        this.emit("flashStatusText", {text: `${msg} ${e ? OK_TEXT : FAILED_TEXT}`});
        return e;
    }
    ...
    async toggleAutoFocus() {
        ...
        this.emit("flashStatusText", {text: `${msg} ${FAILED_TEXT}`});
    ...
    async setFocusZone(id) {
        ...
        if (id==6) {
            ...
            this.emit("flashStatusText", {text: `${msg} 'point' not allowed. Ignoring.`});
    ...
    async reloadCameraSettings() {
        ...
        this.emit("flashStatusText", {text: msg, timeout: 0});
    ...
    async rebootCamera() {
        ...
        this.emit("flashStatusText", {text: msg, timeout: 0});        // leave this message on
        const e = await this.callAPI(j_camera_reboot);

        if (e) {
            this.emit("flashStatusText", {text: `${msg}  Shutting down now...`, timeout: 0});
            await sleep(30000);         // PTZ startup 'dance' occurs at about 26 seconds
            this.emit("flashStatusText", {text: `${msg}  Starting up...`, timeout: 0});
            ...
            while (true) {
                ...
                if (cameraResponse.rsp == "succeed" ) {
                    this.emit("flashStatusText", {text: "Camera on.  Please wait for image."});
                    break;
                };
                i--;
                if (i <= 0) {
                    this.emit("flashStatusText", {text: "Camera unresponsive.  Try restarting camera a different way.", timeout: 0});
                    break;
                }            
            }
        } else {
            this.emit("flashStatusText", {text: "Restart instruction failed.  Try restarting camera a different way.", timeout: 0});
        }
        return e;
    }

    async wakeUpCamera() {
        this.emit("flashStatusText", {text: "Sending power_on instruction to camera.", timeout: 0});
        const e = await this.callAPI(j_camera_wakeup);
        if (e) {
            this.emit("flashStatusText", {text: "Camera power_on OK." });
        } else {
            this.emit("flashStatusText", {text: "Camera unresponsive.  Try restarting it.", timeout: 0});
        };
        return e;
    }
